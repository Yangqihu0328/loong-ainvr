...
inline const char* CascadeModeToString(CascadeMode m) {
  switch (m) {
    case CascadeMode::kPrimary:  return "primary";
    case CascadeMode::kCrop:     return "crop";
    case CascadeMode::kParallel: return "parallel";
  }
  return "unknown";
}
...
struct CascadeStep {
  ...
  std::vector<std::string> trigger_classes;  // Trigger only on these classes (empty = all)
  float confidence_threshold = 0.5F;
  float roi_expand_ratio = 0.1F;            // Expand ROI box by this ratio before cropping
};
...
///
/// Results from primary and parallel steps populate `AnalysisResult::detections`.
/// Results from crop steps populate `AnalysisResult::secondary_results`.
class ModelCascade {
 public:
  ...
  static std::vector<uint8_t> CropRoi(const uint8_t* image_data,
 ...
 private:
  ...
  bool RunPrimaryOrParallel(const StepEntry& entry,
                            const uint8_t* image_data,
                            int width, int height,
                            AnalysisResult& result);

  bool RunCrop(const StepEntry& entry,
               const uint8_t* image_data,
               int width, int height,

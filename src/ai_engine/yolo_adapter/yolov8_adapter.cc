...
bool YoloV8Adapter::PreProcess(const uint8_t* image_data, int width, int height,
...
bool YoloV8Adapter::PostProcess(const std::vector<float>& output_data,
  ...
  for (int i = 0; i < num_detections; ++i) {
    ...
    for (int c = 0; c < num_classes; ++c) {
      float score = output_data[(4 + static_cast<size_t>(c)) * nd + idx];
      if (score > best_score) {
        best_score = score;
        best_class = c;
      }
...
void YoloV8Adapter::LetterboxResize(const uint8_t* src, int src_w, int src_h,

...
class AnalyticsStore {
 public:
  ...
  std::vector<HourlyAggregate> QueryHourly(int channel_id,
  ...
  std::vector<TrendBucket> GetTrends(int64_t start_ms, int64_t end_ms,
                                     TimeGranularity granularity,
                                     int channel_id = -1,
                                     const std::string& rule_type = "");

  /// Get heatmap data from stored detection positions.
  std::vector<HeatmapCell> GetHeatmap(int64_t start_ms, int64_t end_ms,
                                      int channel_id = -1, int grid_cols = 20,
                                      int grid_rows = 15,
                                      int image_width = 1920,
                                      int image_height = 1080);

  /// Get peak hour distribution.
  std::vector<PeakHourEntry> GetPeakHours(int64_t start_ms, int64_t end_ms,
                                          int channel_id = -1);

  /// Store a detection position for heatmap accumulation.
  bool LogDetectionPosition(int channel_id, int64_t timestamp, float center_x,
                            float center_y);
